...
class ChsMonitorApiControllerTest {
    ...
    void shouldReturnListOfSubscriptions() throws Exception {
        when(subscriptionService.getSubscriptions(anyString(), any(Pageable.class),
                eq(ERIC_PASSTHROUGH))).thenReturn(SUBSCRIPTIONS);
    ...
    void shouldReturnStatus416() throws Exception {
        when(subscriptionService.getSubscriptions(anyString(), any(Pageable.class),
                eq(ERIC_PASSTHROUGH))).thenThrow(new ArrayIndexOutOfBoundsException());
